// ============================================
// RAILWAY GATEWAY - FULL COMPLETE (FIXED CORE)
...
class GatewayServer {
  constructor() {
    ...
    this.isDirectRoute = false; // Flag penanda mode DIREK
  }
...
<body class="text-slate-300 min-h-screen flex flex-col justify-between selection:bg-blue-600 selection:text-white">
      ...
      <div class="bg-[#0d0e16] border border-slate-900 rounded-xl p-6 space-y-4">
        ...
        <div class="space-y-3 max-h-[420px] overflow-y-auto pr-1">
          ...
          <div class="bg-[#10121d] border border-slate-900/60 p-4 rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-3 hover:bg-[#121524] transition">
            <div>
              <span class="text-xs bg-red-500/10 text-red-400 px-2 py-0.5 rounded font-bold border border-red-500/20">RAILWAY DIRECT</span>
              <p class="text-sm font-semibold text-slate-200 mt-2">${protocolWs}://${currentHost}/DIREK</p>
            </div>
            <button onclick="copyText('${protocolWs}://${currentHost}/DIREK')" class="text-xs bg-[#171a29] border border-slate-800 text-slate-400 hover:text-white hover:border-red-500 px-3 py-1.5 rounded transition flex items-center gap-1.5 active:scale-95">
          ...
          <div>
            ...
            <div class="flex gap-2">
              <select id="pathSelect" 
                ...
                <option value="/DIREK">⚡ /DIREK (Railway Direct IP)</option>
                <option value="/G-SG">🇸🇬 /G-SG (Hardcode SG Group)</option>
                <option value="/SG-DO">💧 /SG-DO (Hardcode DigitalOcean)</option>
  ...
  <footer class="border-t border-slate-950 bg-[#07080d] px-6 py-4 text-center text-xs text-slate-600">
    <div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
      <p>&copy; 2026 RAILWAY GATEWAY. ALL SYSTEM VECTORS OPERATIONAL.</p>
  ...
  <script>
    ...
    // Tetap menggunakan password dinamis bawaan asli script temanmu
  ...
  async getPrxList(prxBankUrl) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Proteksi jika PRX_BANK_URL malah diisi file format KV objek biar tidak crash saat .map()
  ...
  async handleWebSocketConnection(ws, request) {
    try {
      ...
      // 1. FITUR BARU: Path /DIREK (Bypass mentah lurus keluar lewat IP Server Railway)
      if (path.toUpperCase() === '/DIREK') {
        this.isDirectRoute = true;
        this.prxIP = "";
        console.log(`[ROUTE MATCH] Active Mode -> RAILWAY DIRECT OUTBOUND`);
        await this.websocketHandler(ws);
        return;
      }

      // 2. FITUR BARU: Deteksi Custom Proxy Hardcode di atas (/G-SG, /SG-DO, dll)
      ...
      if (customMatch) {
        ...
        if (CUSTOM_PROXY[customKey] && CUSTOM_PROXY[customKey].length > 0) {
          ...
          console.log(`[ROUTE MATCH] Custom Hardcode Key: ${customKey} -> ${this.prxIP}`);
          await this.websocketHandler(ws);
          return;
        }
      }
  ...
  async websocketHandler(ws) {
    ...
    ws.on('message', async (message) => {
      try {
        ...
        // 100% Menggunakan sniffer bawaan asli temanmu yang terbukti tembus di Railway
  ...
  async handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, responseHeader, log) {
    ...
    // JIKA PATH /DIREK AKTIF: Buang lurus keluar lewat IP Server Railway asli
    ...
    // JIKA LEWAT PATH PROXY: Belokkan koneksi paksa ke IP Proxy (Trik antibocor IP Railway)
    const retry = async () => {
      try {
