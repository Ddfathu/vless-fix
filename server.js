...
class GatewayServer {
  constructor() {
    ...
    this.isDirectRoute = true; // Dipaksa true agar selalu lurus pakai IP Railway
  }
...
<body class="text-slate-300 min-h-screen flex flex-col justify-between p-6">
  ...
  <header class="border-b border-slate-900 pb-4 flex justify-between items-center max-w-7xl w-full mx-auto">
    <div>
      ...
      <p class="text-xs text-slate-500">CORE TUNNEL HYBRID LOADED</p>
    </div>
  ...
  <main class="max-w-7xl w-full mx-auto my-6 space-y-6 flex-grow">
    ...
    <div class="bg-[#0d0e16] border border-slate-900 rounded-xl p-6 space-y-1">
      <p class="text-xs text-slate-400">
        <span class="text-emerald-400 font-bold">INFO UNIVERSAL:</span> Kamu bebas mengisi kolom path di aplikasi VPN dengan nama atau kata apa saja, semuanya otomatis terkoneksi langsung via server Railway.
      </p>
    </div>
  ...
  async handleWebSocketConnection(ws, request) {
    // Sesuai kode universal final: langsung izinkan path kata apa saja masuk
  ...
  async websocketHandler(ws) {
    ...
    ws.on('message', async (message) => {
      try {
        ...
        // Menggunakan logic pembaca Shadowsocks asli bawaan temanmu yang terbukti meloloskan handshake awal
  ...
  async handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, responseHeader) {
    ...
    // Semua dipaksa lurus keluar via IP Railway murni (Tembus Antilelet)
    try {
  ...
  // Fungsi pembaca sakti bawaan temanmu yang meloloskan traffic WebSocket Railway di DarkTunnel
  readSsHeader(buf) {
    try {
